  ...
  <div class="tbl">
    ...
    <div id="t-charts" style="display:none">
      <div class="charts-grid">
        <div class="chart-card"><div class="chart-title">NMV Mensual</div><svg id="c-nmv" width="100%" style="overflow:visible"></svg><div class="chart-sub" id="c-nmv-sub"></div></div>
        <div class="chart-card"><div class="chart-title">Órdenes Mensual</div><svg id="c-ord" width="100%" style="overflow:visible"></svg></div>
...
// ── CHARTS ────────────────────────────────────────────────────────────────────
function getW(id){{const r=document.getElementById(id).getBoundingClientRect();return r.width||380;}}

function drawLineArea(svgId,vals,labels,color,fmt){{
  const W=getW(svgId),H=180,PL=48,PR=12,PT=16,PB=32;
  const cW=W-PL-PR,cH=H-PT-PB;
  const pts=vals.map((v,i)=>{{return{{v,i,lbl:labels[i]}}}}).filter(p=>p.v!=null);
  if(pts.length<2)return;
  const ys=pts.map(p=>p.v),mnY=Math.min(...ys),mxY=Math.max(...ys),rng=mxY-mnY||mxY||1;
  const N=vals.length;
  const sx=i=>PL+i/(N-1)*cW;
  const sy=v=>PT+cH-(v-mnY)/rng*cH;
  let svg=`<defs><linearGradient id="ag${{svgId}}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${{color}}" stop-opacity="0.25"/><stop offset="100%" stop-color="${{color}}" stop-opacity="0.02"/></linearGradient></defs>`;
  // grid lines
  for(let g=0;g<4;g++){{
    const yg=PT+cH*g/3;
    const val=mxY-(rng*g/3);
    const lbl=fmt==='pct'?(val*100).toFixed(1)+'%':fmt==='money'?(val>=1e6?'$'+(val/1e6).toFixed(1)+'M':'$'+(val/1e3).toFixed(0)+'K'):(val>=1e3?(val/1e3).toFixed(0)+'K':Math.round(val));
  ...
  pts.forEach((p,pi)=>{{
    ...
    svg+=`<circle cx="${{sx(p.i).toFixed(1)}}" cy="${{sy(p.v).toFixed(1)}}" r="${{isLast?4:2.5}}" fill="${{isLast?color:'#111827'}}" stroke="${{color}}" stroke-width="1.5"/>`;
    if(isLast){{
      const lbl=fmt==='pct'?(p.v*100).toFixed(1)+'%':fmt==='money'?(p.v>=1e6?'$'+(p.v/1e6).toFixed(1)+'M':'$'+(p.v/1e3).toFixed(0)+'K'):(p.v>=1e3?(p.v/1e3).toFixed(0)+'K':Math.round(p.v));
      svg+=`<text x="${{sx(p.i).toFixed(1)}}" y="${{(sy(p.v)-8).toFixed(1)}}" text-anchor="middle" font-size="10" font-weight="700" fill="${{color}}">${{lbl}}</text>`;
    }}
  }});
  // x axis labels
...
function drawBars(svgId,vals,labels,color,fmt){{
  const W=getW(svgId),H=180,PL=48,PR=12,PT=16,PB=32;
  const cW=W-PL-PR,cH=H-PT-PB;
  const N=vals.length,gap=4,bW=(cW-gap*(N-1))/N;
  const clean=vals.filter(v=>v!=null),mxY=Math.max(...clean)||1;
  let svg='';
  for(let g=0;g<4;g++){{
    const yg=PT+cH*g/3;
    const val=mxY*(1-g/3);
    const lbl=fmt==='num'?(val>=1e6?'$'+(val/1e6).toFixed(1)+'M':(val>=1e3?(val/1e3).toFixed(0)+'K':Math.round(val))):(val>=1e3?(val/1e3).toFixed(0)+'K':Math.round(val));
    svg+=`<line x1="${{PL}}" y1="${{yg.toFixed(1)}}" x2="${{W-PR}}" y2="${{yg.toFixed(1)}}" stroke="#1c2a3e" stroke-width="1"/>`;
    svg+=`<text x="${{PL-5}}" y="${{(yg+4).toFixed(1)}}" text-anchor="end" font-size="9" fill="#3a5070">${{lbl}}</text>`;
  }}
  vals.forEach((v,i)=>{{
    ...
    const bH=v/mxY*cH;
    const isLast=i===N-1;
    const col=isLast?color:color+'99';
    svg+=`<rect x="${{x.toFixed(1)}}" y="${{(PT+cH-bH).toFixed(1)}}" width="${{bW.toFixed(1)}}" height="${{bH.toFixed(1)}}" rx="3" fill="${{col}}"/>`;
    const lbl=fmt==='num'?(v>=1e6?(v/1e6).toFixed(1)+'M':(v>=1e3?(v/1e3).toFixed(0)+'K':v)):v;
    if(isLast) svg+=`<text x="${{(x+bW/2).toFixed(1)}}" y="${{(PT+cH-bH-5).toFixed(1)}}" text-anchor="middle" font-size="10" font-weight="700" fill="${{color}}">${{lbl}}</text>`;
    svg+=`<text x="${{(x+bW/2).toFixed(1)}}" y="${{(PT+cH+18).toFixed(1)}}" text-anchor="middle" font-size="9" fill="#3a5070">${{labels[i]}}</text>`;
  }});
  document.getElementById(svgId).setAttribute('height',H);
  document.getElementById(svgId).innerHTML=svg;
}}
...
function drawMultiLine(svgId,seriesList,labels){{
  const W=getW(svgId),H=190,PL=48,PR=60,PT=16,PB=32;
  const cW=W-PL-PR,cH=H-PT-PB;
  const N=labels.length;
  const allV=seriesList.flatMap(s=>s.vals.filter(v=>v!=null));
  const mnY=Math.min(...allV),mxY=Math.max(...allV),rng=mxY-mnY||0.01;
  const sx=i=>PL+i/(N-1)*cW;
  const sy=v=>PT+cH-(v-mnY)/rng*cH;
  let svg='';
  for(let g=0;g<4;g++){{
    const yg=PT+cH*g/3;
    const val=mnY+(mxY-mnY)*(1-g/3);
    svg+=`<line x1="${{PL}}" y1="${{yg.toFixed(1)}}" x2="${{W-PR}}" y2="${{yg.toFixed(1)}}" stroke="#1c2a3e" stroke-width="1"/>`;
  ...
  seriesList.forEach((s,si)=>{{
    ...
    if(pts.length<2)return;
    const path=pts.map((p,i)=>(i===0?'M':'L')+sx(p.i).toFixed(1)+','+sy(p.v).toFixed(1)).join(' ');
    svg+=`<path d="${{path}}" fill="none" stroke="${{s.color}}" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>`;
    pts.forEach(p=>{{svg+=`<circle cx="${{sx(p.i).toFixed(1)}}" cy="${{sy(p.v).toFixed(1)}}" r="2" fill="${{s.color}}"/>`;}});
    const last=pts[pts.length-1];
    svg+=`<text x="${{(W-PR+6).toFixed(1)}}" y="${{(sy(last.v)+4).toFixed(1)}}" font-size="9" fill="${{s.color}}" font-weight="600">${{s.label}}</text>`;
  }});
  labels.forEach((lbl,i)=>{{
    svg+=`<text x="${{sx(i).toFixed(1)}}" y="${{(PT+cH+18).toFixed(1)}}" text-anchor="middle" font-size="9" fill="#3a5070">${{lbl}}</text>`;
  }});
...
function buildCharts(){{
  ...
  const ordVals=growthData.find(m=>m.metric==='Órdenes').rows[0].v;
  ...
  drawBars('c-ord',ordVals,MONTHS,'#34c47a','num');
  drawMultiLine('c-fr',[
...
// ── PLAN ──────────────────────────────────────────────────────────────────────
const planData=[
  {{metric:'NMV',fmt:'money',target:null,real:306868637}},
  {{metric:'Órdenes',fmt:'num',target:null,real:33162}},
  {{metric:'Compras',fmt:'num',target:null,real:5262}},
  {{metric:'Fill Rate Items',fmt:'pct',target:null,real:0.8898}},
  {{metric:'Fill Rate Compras',fmt:'pct',target:null,real:0.5930}},
  {{metric:'On Time',fmt:'pct',target:null,real:0.8558}},
  {{metric:'Cancelaciones',fmt:'pct',target:null,real:0.0668}},
  {{metric:'Visitas',fmt:'num',target:null,real:153449}},
  {{metric:'NPS',fmt:'pp',target:null,real:0.15}},
];

function buildPlan(){{
  document.getElementById('h-plan').innerHTML=`<tr>
    <th class="left" style="min-width:200px"><span class="dt">Métrica</span></th>
    <th><span class="rel">objetivo</span><span class="dt">Plan Mar'26</span></th>
    <th class="hl"><span class="rel">real</span><span class="dt">Real Mar'26</span></th>
    <th class="last"><span class="rel">diferencia</span><span class="dt">vs Plan</span></th>
    <th><span class="dt">Notas</span></th></tr>`;
  const tb=document.getElementById('b-plan');tb.innerHTML='';
  planData.forEach(row=>{{
    const tr=document.createElement('tr');
    const rv=fv(row.real,row.fmt);
    const tv=row.target!=null?fv(row.target,row.fmt):'<span style="color:#2c3f5c">—</span>';
    let vs='<td class="last neu">—</td>';
    if(row.target!=null){{
      const d=row.fmt==='pct'||row.fmt==='pp'?(row.real-row.target)*100:(row.real-row.target)/row.target*100;
      const s=d>=0?'+':'';
      const txt=row.fmt==='pct'||row.fmt==='pp'?s+d.toFixed(2)+'pp':s+d.toFixed(1)+'%';
      vs=`<td class="last ${{d>=0?'pos':'neg'}}">${{txt}}</td>`;
    }}
    tr.innerHTML=`<td class="metric left">${{row.metric}}</td><td>${{tv}}</td><td class="hl">${{rv}}</td>${{vs}}<td style="color:#2c3f5c;font-size:11px">—</td>`;
    tb.appendChild(tr);
  }});
}}
